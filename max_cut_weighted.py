...
def max_cut_weighted_flatten_graph(graph:nx.Graph):
    flat_adj_matrix = []
...
@njit
def max_cut_weighted_get_gains(start,end):
...
@njit
def max_cut_weighted_calculate_obj(adj_list,start,end,solution):
...
@njit
def max_cut_weighted_gain_adjustment(adj_list,start,end,gains,selected_element,spins):
        ...
        if mask[node]==1 and node_weights[node]<=budget/2:
            ...
            if density_gain_X >= obj_ratio_X and density_gain_Y >= obj_ratio_Y:
                if density_gain_X >= density_gain_Y:
                    obj_X += gains_X[node]
                    max_cut_weighted_gain_adjustment(adj_list=adj_list,start=start,end=end,gains=gains_X,
                    ...
                    obj_Y += gains_Y[node]
                    max_cut_weighted_gain_adjustment(adj_list=adj_list,start=start,end=end, gains=gains_Y, 
                ...
                obj_Y += gains_Y[node]
                max_cut_weighted_gain_adjustment(adj_list=adj_list,start=start,end=end, 
...
def get_max_obj(adj_list,start,end, nodes,size, node_weights,mask, budget, c_value):
    ...
    # max_gain = 0 
    obj = max_cut_weighted_calculate_obj(adj_list=adj_list,start=start,end=end,solution=solution)
    ...
    # assert max_obj == max_cut_weighted_calculate_obj(adj_list=adj_list,start=start,end=end,solution=solution)
...
def DLA_numba(adj_list,start,end,node_weights,budget,mask,eps=0.1):
    ...
    gains = max_cut_weighted_get_gains(start,end)
    # S_prime = LA(adj_list=adj_list,start=start,end=end,
    #              gains=gains,node_weights=node_weights,budget=budget,mask=mask)
    
    
    
    # tau = max_cut_weighted_calculate_obj(adj_list=adj_list,start=start,end=end,solution=S_prime)
    ...
    S = max(tau,
            max_cut_weighted_calculate_obj(adj_list,start,end,solution_X),
            max_cut_weighted_calculate_obj(adj_list,start,end,solution_Y))
    
    if S == max_cut_weighted_calculate_obj(adj_list,start,end,solution_X):
        best_solution = solution_X.copy()
    
    elif S == max_cut_weighted_calculate_obj(adj_list,start,end,solution_Y):
...
def DLA(graph,budget,ground_set=None):

    adj_list,start,end = max_cut_weighted_flatten_graph(graph=graph)
    N = len(start)
...
if __name__ == "__main__":
    ...
    graph = nx.barabasi_albert_graph(10000, 5)
    graph = assign_normalized_degree_weights(graph)

    
    print(DLA(graph=graph,budget=args.budget))
