...
We agree that absolute runtimes should be reported clearly. For completeness, the table below reports absolute inference time for Influence Maximization (size constraint). A full breakdown by component (feature generation, classifier training, and inference) will be included in the revised paper.
