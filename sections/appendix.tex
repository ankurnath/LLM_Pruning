...
\textcolor{blue}{Table~\ref{tab:runtime_absolute} reports absolute inference runtimes for all evaluated algorithms on Influence Maximization under the size constraint. All algorithms are evaluated under the same computational resources (same number of threads and GPUs) to ensure a fair comparison; the only exception is SS, which requires additional threads on large graphs due to its sequential nature. \llm is 1--4 orders of magnitude faster than classical methods on larger graphs, completing inference in under 3 seconds even for Skitter.

\begin{table}[h!]
\centering
\caption{Absolute inference runtime (seconds) for Maximum Coverage under size constraint. *SS uses additional threads on large graphs to speed up pruning; all other algorithms use the same number of threads and GPUs.}
...
Table~\ref{tab:runtime_breakdown} breaks down the total inference time of \llm into feature extraction and GNN inference components. Feature extraction time grows with graph size, while GNN inference time remains roughly constant across datasets, indicating that the bottleneck on large graphs is feature computation rather than the classifier.
